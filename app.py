...
authenticator = Authenticate(credentials, 'greeninvest_cookie', 'a_strong_secret_key', cookie_expiry_days=30)
...
if st.session_state["authentication_status"]:
    ...
    if input_method == "Manual Input":
        with st.sidebar.form(key='manual_input_form'):
            # Input fields here, pre-filled from current_inputs
            st.form_submit_button("Calculate ESG Score", type="primary", use_container_width=True)
            # ... (Full form logic as in previous versions)
    else: # CSV Upload
        # ... (Full CSV upload logic as in previous versions)
        pass
    ...
    if user_history:
        ...
        process_and_display_data(latest_entry['env_data'], latest_entry['social_data'], latest_entry['gov_data'])
    else:
        st.info("👋 Welcome! Please provide your data in the sidebar to calculate your first ESG score.")
...
# Footer
st.divider()
st.markdown("<p style='text-align: center; color: grey;'>Made with ❤️ for a greener future.</p>", unsafe_allow_html=True)
